...
fun toUploadImage(activity:Activity,uploadImgData: ImageData,provider:String){
    ...
    intent.putExtra(UploadImgData.UPLOADIMG_DATA_KEY,uploadImgData)
    activity.startActivityForResult(intent,REQEUST_CODE_UPLOADIMG)
}

fun toUploadImage(fragment: Fragment,uploadImgData: ImageData,provider:String){
    uploadImgData.provider = provider;
    var intent = Intent(fragment.context,UploadimgActivity::class.java);
    intent.putExtra(UploadImgData.UPLOADIMG_DATA_KEY,uploadImgData)
    fragment.startActivityForResult(intent,REQEUST_CODE_UPLOADIMG)
}